...
const noHiddenRoutes = computed(() => {
  const routesSource = permissionStore.routes && permissionStore.routes.length ? permissionStore.routes : constantRoutes;
  return routesSource.filter((item: any) => !item.meta?.hidden);
});
