  ...
  <div>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible">
      <el-form :model="formData">
        <el-form-item label="Email"><el-input v-model="formData.email"/></el-form-item>
...
import { ref } from 'vue';
import { getNotifyEmailList, addNotifyEmail, updateNotifyEmail, deleteNotifyEmail } from '@/common/apis/notify_email';
...
function openDialog(row?: any) {
  ...
  else { dialogTitle.value = 'Add New'; formData.value = { id: '', email: '', name: '' }; }
  dialogVisible.value = true;
}

async function handleSave() { if (formData.value.id) await updateNotifyEmail(formData.value); else await addNotifyEmail(formData.value); dialogVisible.value = false; fetchList(); }

async function handleDelete(id: string) { await deleteNotifyEmail(id); fetchList(); }
