  ...
  <div>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible">
      <el-form :model="formData">
        ...
        <el-form-item label="Link"><el-input v-model="formData.link"/></el-form-item>
        <el-form-item label="Win Process"><el-input v-model="formData.process_win"/></el-form-item>
        <el-form-item label="Mac Process"><el-input v-model="formData.process_mac"/></el-form-item>
      </el-form>
      <template #footer>
...
import { ref } from 'vue';
import { getVmAppList, addVmApp, updateVmApp, deleteVmApp } from '@/common/apis/vm_app';
...
async function fetchList() {
  const res: any = await getVmAppList({ name: filters.value.name, page: page.value - 1, page_size: pageSize.value });
...
function openDialog(row?: any) {
  ...
  else { dialogTitle.value = 'Add New'; formData.value = { id: '', name: '', link: '', process_win: '', process_mac: '' }; }
  dialogVisible.value = true;
}

async function handleSave() { if (formData.value.id) await updateVmApp(formData.value); else await addVmApp(formData.value); dialogVisible.value = false; fetchList(); }

async function handleDelete(id: string) { await deleteVmApp(id); fetchList(); }
